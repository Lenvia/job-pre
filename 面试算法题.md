...
### <font color=purple>NC24 删除有序链表中重复的元素-II</font>
...
设置flag。意义是删除重复的最后一个元素。 比如 1 1 <font color=red>1</font> 2 3 ...  红色的1需要借助flag删除。

如果cur->val == nxt->val，标记flag，并删除cur。

否则（ cur->val != nxt->val），如果flag ==1 仍然删除 cur。然后记得把flag重置为0

删除cur的代码：（我不知道怎么复用。。重复写了三段。调用函数的话指针不能按值传递。）

```
if(cur==head){  // pre==nullptr
    head = head->next;
    cur = head;
}
else{
...
### ⚡️<font color=orange>NC119 最小的K个数</font>
