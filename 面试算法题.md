...
历该数组两次，处理出每一个学生分别满足左规则或右规则时，最少需要被分得的糖果数量。每个人最终分得的糖果数量即为这两个数量的最大值。
...
public:
    ...
    void quickSort(vector<int>& arr, int left, int right){  // [left, right]
        ...
        int randm = left + rand() % (right-left+1);
        swap(arr[randm], arr[right]);
    ...
    vector<int> MySort(vector<int>& arr) {
        quickSort(arr, 0, arr.size()-1);  // 递归快排
        return arr;
    }
};
```
