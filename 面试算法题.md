...
### ⚠️<font color=red>[lc]149.直线上最多的点数</font>
...
fast 比 slow 超前 n 个节点。当 fast 遍历到链表的末尾时，slow 就恰好处于倒数第 n 个节点。
...
![截屏2022-06-08 14.35.55](https://tva1.sinaimg.cn/large/e6c9d24ely1h30ts5yxw9j21ly08476n.jpg)
...
<img src="https://tva1.sinaimg.cn/large/e6c9d24ely1h39capfp07j217s0igjtb.jpg" alt="截屏2022-06-15 23.21.01" style="zoom:40%;" />

两个左半部分放在一起，两个右半部分放在一起。

<img src="https://tva1.sinaimg.cn/large/e6c9d24ely1h39cezxbk7j219e08kq42.jpg" alt="截屏2022-06-15 23.25.13" style="zoom:40%;" />
...
<img src="https://tva1.sinaimg.cn/large/e6c9d24ely1h39ctkp2dgj20pu04g0ss.jpg" alt="截屏2022-06-15 23.39.14" style="zoom:50%;" />

等价于：（因为随着 i递增，A\[i-1\] 递增，B\[j\] 递减，如果i最大了，那么第二个条件一定满足）

<img src="https://tva1.sinaimg.cn/large/e6c9d24ely1h39ctx0j3yj20i004gaa2.jpg" alt="截屏2022-06-15 23.39.32" style="zoom:50%;" />
...
public:
    double findMedianSortedArrays(vector<int>& nums1, vector<int>& nums2) {
        ...
        while(left<=right){
            ...
            j = (m + n +1)/ 2 - i;
            // printf("%d %d %d %d\n", left, right, i, j);
            // nums1[0 ~ i-1], nums1[i, m-1], nums2[0 ~ j-1], nums2[j, n-1]
...
### ⚡️<font color=dodgerblue>NC42 有重复项数字的全排列</font>
...
### ⚡️<font color=orange>NC46 加起来和为目标值的组合(二)</font>
...
public:
    int partition(vector<int>& vec, int l, int r){  // [l, r]
        int randm = rand()%(r-l+1) + l;  // 随机取一个座位pivot。 （注意是 r-l+1 别写反了！！错好几次了）
...
需要用到 <u>NC51 合并k个已排序的链表</u>  的部分函数
...
先按start进行排序，遍历的时候左边界就可以定下来，然后不断向右扩充。判断条件是 b和c的大小
...
### <font color=red>NC93 设计LRU缓存结构</font>
...
public:
    ...
    map<int, int>record;  // key -> (timestamp)
    // 小顶堆 (timestamp, key)
    priority_queue<pair<int, int>, vector<pair<int, int>>, greater<pair<int, int>>> inq, deq;
    
    
    void update_record(int key){  // 更新key对应的count和timestamp信息
        ++timestamp;
        if(record.count(key)){
            deq.push(make_pair(record[key], key));
            
            record[key] = timestamp;
            inq.push(make_pair(record[key], key));
        }
        else{
            record[key] = timestamp;
            inq.push(make_pair(timestamp, key));
        }
    }
            ...
            if(oper[0]==1){  // set
                if(cache.count(oper[1])){  // 有这个元素
                    ...
                    if(cache.size()<k){  // 不用删除其他的
                        ...
                        // 通过LRU策略返回要删除的key，执行删除然后再插入
                        int dkey = query_LRU();
                        deq.push(make_pair(record[dkey], dkey));
                        cache.erase(dkey);
                        record.erase(dkey);
                        // 插入
                        cache[oper[1]] = oper[2];
    ...
    int maxLength(vector<int>& arr) {
        ...
        while(right<arr.size()){
            if(visited.find(arr[right])==visited.end()){
                ...
                while(visited.find(arr[right])!=visited.end()){  // 不断右移动left，直到删除
                    visited.erase(arr[left]);
                    left++;
                }
                // 到这里不包含 arr[right] 了
                visited[arr[right]] = true;
...
维护一个<font color=red>**单调的队列**</font>（这里为了记录每个元素的有效期，队列里放置的索引）
...
维护一个左栈 lst，遇到栈顶比当前元素nums\[i]大的就弹出（因为栈顶比当前元素都大，后面再往右遍历时nums\[i]就够了），直到遇到比自己小的。
